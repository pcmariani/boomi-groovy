...
Push-Location -Path "$env:BOOMI_GROOVY_HOME"
Write-Debug -Message "pwd: $(pwd)"

Write-Debug -Message "to be executed: groovy $env:BOOMI_GROOVY_HOME\boomi-test.groovy $args -w $workingDir"
groovy "$env:BOOMI_GROOVY_HOME\boomi-test.groovy" $args -w $workingDir
